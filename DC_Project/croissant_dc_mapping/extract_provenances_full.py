...
def fetch_all_provenances(api_key: str, output_file: str = "provenances_full.json") -> None:
    """
    Fetches Provenance nodes and traverses to Dataset and Source levels to extract metadata.
    """
    ...
    print(f"Found {len(provenance_dcids)} provenances.", file=sys.stderr)
    ...
    batch_size = 50
    # temp = client.node.fetch(node_dcids=["dc/base/ACSED5YrSurvey"], expression="->*").to_dict()
    ...
    for i in range(0, len(provenance_dcids), batch_size):
        ...
        for dcid in batch:
            ...
            prov_entry = {
                ...
                "prov_url": get_node_property(node_data, "url"),
    ...
    # temp = client.node.fetch(node_dcids=["dc/s/UsCensusBureau"], expression="->*").to_dict()
    # print(json.dumps(temp, indent=2))
