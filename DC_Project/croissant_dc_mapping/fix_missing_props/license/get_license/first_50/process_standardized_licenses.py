...
domain_license_url_map = {
    ...
    # Fallback mappings
    "www.indec.gob.ar": "https://creativecommons.org/licenses/by/4.0/", # INDEC general is CC BY 4.0
    "www.abs.gov.au": "https://creativecommons.org/licenses/by/4.0/",
    "datacommons.org": "https://datacommons.org/disclaimer",
    "www.ibge.gov.br": "https://creativecommons.org/licenses/by/4.0/", # IBGE general is CC BY 4.0
    "aplicacoes.cidadania.gov.br": "https://creativecommons.org/licenses/by-nd/3.0/",
    "sidra.ibge.gov.br": "https://creativecommons.org/licenses/by/4.0/",
...
def resolve_license_for_entry(entry):
...
def main():
    # 1. Update provenances_full.json
    with open("provenances_full.json", "r") as f:
        full_data = json.load(f)
        
    updated_full_data = []
    for entry in full_data:
        jet_license = resolve_license_for_entry(entry)
        
        # Construct entry preserving order
        new_entry = {}
    ...
    print(f"Successfully processed all {len(updated_full_data)} elements in provenances_full.json")
    
    # 2. Update provenances_licenses.json (with the first 50 elements from the updated full data)
    updated_licenses_data = []
    for entry in updated_full_data[:50]:
        updated_licenses_data.append({
            "dcid": entry.get("dcid"),
    ...
    with open("provenances_licenses.json", "w") as f:
        json.dump(updated_licenses_data, f, indent=2)
        
    print(f"Successfully updated provenances_licenses.json with the first 50 elements.")
