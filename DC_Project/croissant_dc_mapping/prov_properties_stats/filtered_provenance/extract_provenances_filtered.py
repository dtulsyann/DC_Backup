...
def fetch_all_provenances(api_key: str, output_file: str = "provenances_filtered_props.json") -> None:
    ...
    with open('/usr/local/google/home/dtulsyan/DC_Project/croissant_dc_mapping/fix_missing_props/Prov_missing_observations/missing_provenances_list.md', 'r') as f:
        content = f.read()
    excluded_dcids = set()
    for match in re.finditer(r'- `(dc/base/[^`]+)`', content):
        excluded_dcids.add(match.group(1))
    
    provenance_dcids = [dcid for dcid in provenance_dcids if dcid not in excluded_dcids]
    ...
    print(f"Found {total_provenances} provenances after filtering.", file=sys.stderr)
    ...
    stats_file = os.path.join(script_dir, "provenance_missing_stats_filtered.json")
...
if __name__ == "__main__":
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_path = os.path.join(script_dir, "provenances_filtered_props.json")
    fetch_all_provenances(api_key=dc_api_key, output_file=output_path)
